...
class ChunkDoubleSourceSlider(object):
    ...
    def check_lenght(self):
        ...
        for chunk in check_cvs:
            ...
            t_size += size
            del chunk
        print('Size of the dataset is {:.3f} M rows.'.format(t_size/10 ** 6))
        if t_size > self.ram:  # IF dataset is too large for memory
            print('It is too large to fit in memory so it will be loaded in chunkes of size {:}.'.format(self.chunksize))
        else:
            print('This size can fit the memory so it will load entirely')
...
class ChunkDoubleSourceSlider2(object):
    ...
    def check_length(self):
        ...
        for chunk in check_cvs:
            ...
            del chunk
        print('Size of the dataset is {:.3f} M rows.'.format(self.total_size / 10 ** 6))
